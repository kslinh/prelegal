  ...
  return (
    <div className="flex h-full flex-col rounded-lg border border-gray-200 bg-white p-6 shadow-sm transition-shadow hover:shadow-md">
      ...
      <div className="flex gap-2 pt-4 border-t border-gray-100">
        <Link href={`/templates/${entry.id}`} className="flex-1">
          <button className="w-full px-3 py-2 text-xs font-medium text-[#209dd7] hover:bg-blue-50 rounded transition-colors">
            View
          </button>
        </Link>
        <Link href={`/templates/${entry.id}/chat`} className="flex-1">
          <button className="w-full px-3 py-2 text-xs font-medium text-white bg-[#209dd7] hover:bg-[#1a7fb0] rounded transition-colors">
            Create with AI
          </button>
        </Link>
      </div>
